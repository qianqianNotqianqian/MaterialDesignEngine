...
JNIEXPORT jstring JNICALL Java_xzr_perfmon_JniTools_getFps
    ...
    if (!read_file_str("/sys/devices/virtual/graphics/fb0/measured_fps", (char *) &fps))
        return (*env)->NewStringUTF(env, (const char *) &fps);

    if (!read_process_str("cat /sys/class/drm/sde-crtc-0/measured_fps | awk '{print $2}'",
                          (char *) &fps))
        return (*env)->NewStringUTF(env, (const char *) &fps);
