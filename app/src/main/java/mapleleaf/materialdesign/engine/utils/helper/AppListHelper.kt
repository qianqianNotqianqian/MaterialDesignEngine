...
//            }
     ...
     * 检查已安装版本
     */
    private fun checkInstall(backupInfo: PackageInfo): String {
        try {
    ...
    fun getBootableApps(
        ...
        for (i in packageInfos.indices) {
            ...
            item.appType =
                if ((applicationInfo.flags and ApplicationInfo.FLAG_SYSTEM) == 0) AppInfo.AppType.USER else AppInfo.AppType.SYSTEM
            try {
                val packageInfo = packageManager.getPackageInfo(applicationInfo.packageName, 0)
                item.versionName = packageInfo.versionName
                item.versionCode = packageInfo.versionCode
    ...
    fun getBackupedAppList(): ArrayList<AppInfo> {
        val dirPath = CommonCommands.AbsBackUpDir
