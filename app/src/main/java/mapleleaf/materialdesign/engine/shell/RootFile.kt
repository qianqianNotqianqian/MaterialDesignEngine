...
import mapleleaf.materialdesign.engine.shared.RootFileInfo
...
object RootFile {
    ...
    fun fileNotEmpty(path: String): Boolean {
        return KeepShellPublic.doCmdSync("if [[ -f \"$path\" ]] && [[ -s \"$path\" ]]; then echo 1; fi;")
            .equals("1")
    }
    ...
    fun list(path: String): ArrayList<RootFileInfo> {
        val absPath =
            if (path.endsWith("/")) path.subSequence(0, path.length - 1).toString() else path
