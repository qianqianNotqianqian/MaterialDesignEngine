...
class SwapUtils(private val context: Context) {
    ...
    private var swapControlScript =
        FileWrite.writePrivateShellFile(
            "sys_modify/swap_control.sh",
            "sys_modify/swap_control.sh",
            context
        )
    private var swapForceKswapdScript: String? = null
    // private var zramControlScript = FileWrite.writePrivateShellFile("sys_modify/zram_control.sh", "sys_modify/zram_control.sh", context)

    // 是否已创建swapfile文件
    private val swapExists: Boolean
        get() {
    ...
    val swapFileSize: Int
        get() {
            if (swapExists) {
                ...
                var size = 0L
                try {
                    size = KeepShellPublic.doCmdSync("ls -l /data/swapfile | awk '{ print \$5 }'")
                        .toLong()
                } catch (ex: Exception) {
                    try {
    ...
    val zramEnabled: Boolean
        get() {
            return KeepShellPublic.doCmdSync("cat /proc/swaps | grep /block/zram0")
                .contains("/block/zram0")
        }
    ...
    fun forceKswapd(level: Int): String {
        if (swapForceKswapdScript == null) {
            swapForceKswapdScript = FileWrite.writePrivateShellFile(
                "sys_modify/force_compact.sh",
                "sys_modify/force_compact.sh",
                context
            )
        ...
        if (swapForceKswapdScript != null) {
            return KeepShellPublic.getInstance("swap-clear", true)
                .doCmdSync("sh $swapForceKswapdScript $level")
        }
        return "Fail!"
    }
    ...
    val swapUsedSize: Int
        get() {
            ...
            for (row in procSwaps) {
                if (row.startsWith("/swapfile ") || row.startsWith("/data/swapfile ") || (loopName != null && row.contains(
                        loopName
                    ))
                ) {
