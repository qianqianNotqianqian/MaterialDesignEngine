 ...
 */
object FileWrite {
    private val SDCardDir: String = Environment.getExternalStorageDirectory().absolutePath
    ...
    fun writeFile(context: Context, file: String, hasExtName: Boolean): String? {
        ...
        try {
            ...
            val filePath = baseUrl + if (hasExtName)
                ...
                file.substring(
                    0,
                    if (file.lastIndexOf(".") > 0) file.lastIndexOf(".") else file.length
                )
    ...
    private fun getPrivateFileDir(context: Context): String {
    ...
    fun writePrivateFile(
        ...
        try {
            ...
            fileOutputStream.close()
            inputStream.close()
            val wdFile = File(filePath)
            wdFile.setWritable(true)
            wdFile.setExecutable(true)
            wdFile.setReadable(true)
            return filePath
    ...
    private fun parseText(context: Context, fileName: String): ByteArray {
        try {
            ...
            val codes =
                String(datas, 0, len).replace(Regex("\r\n"), "\n").replace(Regex("\r\t"), "\t")
