package mapleleaf.materialdesign.engine.services
 ...
 * 后台编译应用
 */
class CompileService : IntentService("engine-compile") {
    companion object {
        var compiling = false
    }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            nm.notify(
                ...
                NotificationCompat.Builder(this, "vtool-compile")
                    .setSmallIcon(R.drawable.process_android)
            ...
            nm.notify(
                990,
                NotificationCompat.Builder(this).setSmallIcon(R.drawable.process_android)
                    .setSubText(title)
    ...
    private fun updateNotification(
        ...
        val builder = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            NotificationCompat.Builder(this, "engine-compile")
        } else {
    ...
    override fun onHandleIntent(intent: Intent?) {
        ...
        */
        mWakeLock = mPowerManager.newWakeLock(PARTIAL_WAKE_LOCK, "engine:CompileService");
        ...
        if (intent != null) {
            if (intent.action == getString(R.string.engine_speed_compile)) {
                compile_method = "speed"
            } else if (intent.action == getString(R.string.engine_speed_profile_compile)) {
                compile_method = "speed-profile"
            } else if (intent.action == getString(R.string.engine_everything_compile)) {
                compile_method = "everything"
            } else if (intent.action == getString(R.string.engine_reset_compile)) {
        ...
        if (compile_method == "reset") {
            ...
            for (packageName in packageNames) {
                if (true) {
                    updateNotification(
                        getString(R.string.dex2oat_reset_running),
                        packageName,
                        total,
                        current
                    )
                    cmdBuilder.append("am broadcast -n com.mapleaf.md_engine/com.mapleaf.md_engine.ReceiverCompileState --ei current $current --ei total $total --es packageName $packageName\n")
            ...
            cmdBuilder.append("am broadcast -n com.mapleaf.md_engine/com.mapleaf.md_engine.ReceiverCompileState --ei current $total --ei total $total --es packageName OK\n")
            val cache = "/dex2oat/reset.sh"
            if (FileWrite.writePrivateFile(
                    cmdBuilder.toString().toByteArray(Charset.defaultCharset()),
                    cache,
            ...
            keepShell.tryExit()
            compileCanceled = true
            toast("重置过程中手机会有点卡，请耐心等待~")
        } else {
            for (packageName in packageNames) {
                if (true) {
                    updateNotification(
                        getString(R.string.dex2oat_compiling) + "[" + compile_method + "]",
