...
open class DialogFullScreen(private val layout: Int, private val darkMode: Boolean) :
    DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            return Dialog(
                requireActivity(),
                if (themeResId != 0) themeResId else R.style.dialog_full_screen_light
            )
        } else {
    ...
    init {
        themeResId =
            if (darkMode) R.style.dialog_full_screen_dark else R.style.dialog_full_screen_light
    }
}