...
class AdapterProcess(
    ...
    private fun keywordSearch(item: ProcessInfo, text: String): Boolean {
        return item.friendlyName.toString().toLowerCase().contains(text) || item.name.toString()
            .toLowerCase().contains(text) || item.user.toString().toLowerCase()
            .contains(text) || item.command.toString().toLowerCase()
            .contains(text) || item.cmdline.toString().toLowerCase().contains(text)
    }
    ...
    private fun isAndroidProcess(processInfo: ProcessInfo): Boolean {
        return (processInfo.command.contains("app_process") && !processInfo.name.isNullOrEmpty() && regexPackageName?.matches(
            processInfo.name
        ) == true)
    }
        ...
        for (item in processes) {
            if (isAndroidProcess(item)) {
                if (nameCache.containsKey(item.name)) {
                    item.friendlyName = nameCache[item.name]
                } else {
                    val name = if (item.name.contains(":")) item.name.substring(
                        0,
                        item.name.indexOf(":")
    ...
    private fun updateRow(position: Int, view: View) {
        ...
        view.run {
            if (processInfo.friendlyName == processInfo.name) {
                findViewById<TextView>(R.id.ProcessFriendlyName).text =
                    keywordHeightLight(processInfo.friendlyName)
                ...
                findViewById<TextView>(R.id.ProcessName).run {
                    ...
                    text = keywordHeightLight(processInfo.name)
                }
            }
            findViewById<TextView>(R.id.ProcessPID).text = processInfo.pid.toString()
            findViewById<TextView>(R.id.ProcessCPU).text =
                "" + processInfo.getCpu() + "%"
            if (processInfo.res > 8192) {
                findViewById<TextView>(R.id.ProcessRES).text =
                    "" + (processInfo.res / 1024).toInt() + "MB"
            } else {
                findViewById<TextView>(R.id.ProcessRES).text =
                    "" + processInfo.res + "KB"
            }
